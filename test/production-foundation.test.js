...
test('durable change feed survives database and server restart', async (t) => {
  ...
  const write = await fetch(`${firstAddress.url}/collections/items/a`, {
    method: 'PUT',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ value: 1, updatedAt: '2026-08-28T12:00:00.000Z' })
  });
  ...
  const pulled = await fetch(`${secondAddress.url}/replicate/pull`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
...
test('replication push is idempotent for the same changeId', async (t) => {
  ...
  const change = {
    ...
    originDatabaseId: 'remote-a',
    collection: 'items',
    type: 'upsert',
    record: { id: 'same', value: 7, updatedAt: '2026-08-28T12:00:00.000Z' }
  };
  const packet = createReplicationPacket({ from: 'remote-a', changes: [change] });

  let response = await fetch(`${address.url}/replicate/push`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  ...
  response = await fetch(`${address.url}/replicate/push`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(packet)
  });
...
test('HTTP boundary rejects oversized and malformed request bodies', async (t) => {
  ...
  let response = await fetch(`${address.url}/collections/items`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
  ...
  response = await fetch(`${address.url}/collections/items`, {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: '{not-json'
  });
...
test('internal exceptions are observable without leaking exception messages to clients', async (t) => {
  ...
  const service = createSyncioServer({
    db: state.db,
    policies: allowAll,
    authenticate: async () => { throw new Error('credential-secret-should-not-leak'); },
