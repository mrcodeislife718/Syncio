...
import { getPath } from './document.js';
...
export class ProductionSyncioDatabase {
  ...
  storageStatus() { return { ...this.base.storageStatus(), capabilities: { schemas: Object.keys(this.metadata.schemas).length, ttlIndexes: Object.keys(this.metadata.ttl).length, textIndexes: Object.keys(this.metadata.text).length, geoIndexes: Object.keys(this.metadata.geo).length } }; }
  ...
  async defineSchema(collection, schema, { mode = 'enforced' } = {}) {
    ...
    if (mode === 'enforced') for (const record of this.base.collection(collection).all()) validateAgainstSchema(record, schema, '$');
  ...
  getSchema(collection) { return this.metadata.schemas[collection] ? clone(this.metadata.schemas[collection]) : null; }

  async defineTTLIndex(collection, field, { expireAfterSeconds = 0 } = {}) {
    validateCollectionName(collection); validateField(field);
  ...
  async defineTextIndex(collection, fields, { name = `text_${normalizeFields(fields).join('_')}` } = {}) {
    validateCollectionName(collection); const normalized = normalizeFields(fields); validateIndexName(name);
    this.metadata.text[indexKey(collection, name)] = { collection, name, fields: normalized };
    this.#rebuildTextIndex(this.metadata.text[indexKey(collection, name)]);
    await this.#persistMetadata();
    return clone(this.metadata.text[indexKey(collection, name)]);
  }

  async defineGeoIndex(collection, field, { name = `geo_${field}` } = {}) {
    validateCollectionName(collection); validateField(field); validateIndexName(name);
    this.metadata.geo[indexKey(collection, name)] = { collection, name, field };
    this.#rebuildGeoIndex(this.metadata.geo[indexKey(collection, name)]);
    await this.#persistMetadata();
    return clone(this.metadata.geo[indexKey(collection, name)]);
  }
  ...
  collection(name) {
    ...
    const db = this;
    const afterWrite = () => db.#rebuildCollectionSpecialIndexes(name);
    return Object.freeze({
      async insert(value) { db.#validateRecord(name, value); const result = await baseCollection.insert(value); afterWrite(); return result; },
      async upsert(value) { db.#validateRecord(name, value); const result = await baseCollection.upsert(value); afterWrite(); return result; },
      get: (id) => baseCollection.get(id),
      all: () => baseCollection.all(),
      async remove(id) { const result = await baseCollection.remove(id); if (result) afterWrite(); return result; },
      ...
      async update(id, operators) {
        const existing = baseCollection.get(id);
        if (!existing) return null;
        const { applyUpdateOperators } = await import('./document.js');
        const next = applyUpdateOperators(existing, operators);
  ...
  search(collection, query, { index, limit = 20 } = {}) {
    ...
    for (const term of terms) for (const [id, weight] of store.postings.get(term) ?? []) scores.set(id, (scores.get(id) ?? 0) + weight);
    return [...scores.entries()].sort((a,b)=>b[1]-a[1] || a[0].localeCompare(b[0])).slice(0, limit).map(([id, score]) => ({ score, record: this.base.collection(collection).get(id) })).filter((row)=>row.record);
  }

  near(collection, point, { index, maxDistanceMeters = Infinity, limit = 20 } = {}) {
    validateCollectionName(collection); const center = normalizePoint(point);
    ...
    return rows.slice(0, limit).map(({id,distanceMeters})=>({distanceMeters, record:this.base.collection(collection).get(id)})).filter((row)=>row.record);
  }
  ...
  async sweepExpired({ now = Date.now(), limit = 10_000 } = {}) {
    ...
    const expired = [];
    for (const [collection, definition] of Object.entries(this.metadata.ttl)) {
      for (const record of this.base.collection(collection).all()) {
        if (expired.length >= limit) break;
        const raw = getPath(record, definition.field);
        const at = timestamp(raw);
        if (at !== null && at + definition.expireAfterSeconds * 1000 <= now) expired.push({ collection, id: record.id });
      }
    }
    if (!expired.length) return { removed: 0, records: [] };
    await this.transaction(async (tx) => { for (const item of expired) tx.collection(item.collection).remove(item.id); });
  ...
  async replaceState(state) {
    for (const [collection, records] of Object.entries(state?.collections ?? {})) for (const record of Object.values(records ?? {})) this.#validateRecord(collection, record);
    const result = await this.base.replaceState(state);
    this.#rebuildSpecialIndexes();
    return result;
  }

  async close() { await this.metadataQueue; await this.base.close(); }
  ...
  #resolveTextIndex(collection, name) {
    ...
    if (candidates.length !== 1) throw Object.assign(new Error(name ? 'text index not found' : 'text index name required when collection has zero or multiple text indexes'), { code:'SYNCIO_TEXT_INDEX_NOT_FOUND' });
    return candidates[0];
  }
  ...
  #resolveGeoIndex(collection, name) {
    ...
    if (candidates.length !== 1) throw Object.assign(new Error(name ? 'geo index not found' : 'geo index name required when collection has zero or multiple geo indexes'), { code:'SYNCIO_GEO_INDEX_NOT_FOUND' });
    return candidates[0];
  }

  #rebuildSpecialIndexes() {
    this.textIndexes.clear(); this.geoIndexes.clear();
  ...
  #rebuildTextIndex(definition) {
    ...
    for (const record of this.base.collection(definition.collection).all()) {
      ...
      for (const field of definition.fields) for (const term of tokenize(flattenText(getPath(record, field)))) counts.set(term, (counts.get(term) ?? 0) + 1);
      for (const [term, weight] of counts) { const ids = postings.get(term) ?? new Map(); ids.set(record.id, weight); postings.set(term, ids); }
    }
  ...
  #rebuildGeoIndex(definition) {
    ...
    for (const record of this.base.collection(definition.collection).all()) {
      ...
      try { points.set(record.id, normalizePoint(raw)); } catch { /* invalid geo values remain normal documents but are absent from geo index */ }
    }
...
export async function openProduction(file, options) { return ProductionSyncioDatabase.open(file, options); }
...
function normalizeMetadata(value) {
  ...
  for (const [collection, def] of Object.entries(metadata.schemas)) { validateCollectionName(collection); if (!['optional','enforced'].includes(def.mode)) throw new Error('invalid schema mode'); validateSchemaDefinition(def.schema); }
  for (const [collection, def] of Object.entries(metadata.ttl)) { validateCollectionName(collection); validateField(def.field); if (!Number.isFinite(def.expireAfterSeconds)||def.expireAfterSeconds<0) throw new Error('invalid TTL definition'); }
...
function timestamp(value) { if (typeof value === 'number' && Number.isFinite(value)) return value; if (typeof value === 'string') { const n = Date.parse(value); return Number.isFinite(n) ? n : null; } if (value instanceof Date) return value.getTime(); return null; }
...
function validateSchemaDefinition(schema) { if (!schema || typeof schema !== 'object' || Array.isArray(schema)) throw new TypeError('schema must be an object'); if (schema.type) normalizeTypes(schema.type); if (schema.required && (!Array.isArray(schema.required)||schema.required.some((item)=>typeof item!=='string')) throw new TypeError('schema.required must be string array'); if (schema.properties) { if (typeof schema.properties!=='object'||Array.isArray(schema.properties)) throw new TypeError('schema.properties must be object'); for (const child of Object.values(schema.properties)) validateSchemaDefinition(child); } if (schema.items) validateSchemaDefinition(schema.items); }
...
export function validateAgainstSchema(value, schema, location = '$') {
  ...
  if (schema.enum && (!Array.isArray(schema.enum)||!schema.enum.some((candidate)=>JSON.stringify(candidate)===JSON.stringify(value)))) throw schemaError(location,'value is not in enum');
  if (typeof value === 'string') { if (Number.isSafeInteger(schema.minLength)&&value.length<schema.minLength) throw schemaError(location,'string shorter than minLength'); if (Number.isSafeInteger(schema.maxLength)&&value.length>schema.maxLength) throw schemaError(location,'string longer than maxLength'); if (schema.pattern && !(new RegExp(schema.pattern,'u')).test(value)) throw schemaError(location,'string does not match pattern'); }
  if (typeof value === 'number') { if (Number.isFinite(schema.minimum)&&value<schema.minimum) throw schemaError(location,'number below minimum'); if (Number.isFinite(schema.maximum)&&value>schema.maximum) throw schemaError(location,'number above maximum'); }
  if (Array.isArray(value)) { if (Number.isSafeInteger(schema.minItems)&&value.length<schema.minItems) throw schemaError(location,'array shorter than minItems'); if (Number.isSafeInteger(schema.maxItems)&&value.length>schema.maxItems) throw schemaError(location,'array longer than maxItems'); if (schema.items) value.forEach((item,i)=>validateAgainstSchema(item,schema.items,`${location}[${i}]`)); }
