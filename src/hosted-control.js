...
export async function createHostedControlServer({ controlPlane, sessionSecret, sessionKeys, activeSessionKeyId, sessionRevocationFile, billingWebhookSecret, rateLimit={capacity:120,refillPerSecond:2,maxKeys:10_000}, observe=()=>undefined }={}) {
  ...
  const revocations=sessionRevocationFile?await DurableRevocationLedger.open(path.resolve(sessionRevocationFile),sessions):null;
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(req.method==='POST'&&url.pathname==='/v1/session/revoke'){if(!revocations)return reply(res,503,{error:'session_revocation_not_configured',requestId},observe,{requestId,req,started});await revocations.revokeToken(session.jti,{expiresAt:session.exp});return reply(res,200,{revoked:true},observe,{requestId,req,started});}
      ...
      if(req.method==='DELETE'&&url.pathname==='/v1/account'){if(revocations)await revocations.revokeSubject(account.id);await controlPlane.deleteAccount(account.id);return reply(res,200,{deleted:true},observe,{requestId,req,started});}
      ...
      const tokenMatch=url.pathname.match(/^\/v1\/projects\/([^/]+)\/token$/);if(req.method==='POST'&&tokenMatch){const projectId=decodeURIComponent(tokenMatch[1]);const project=controlPlane.project(projectId);if(!project||project.accountId!==account.id)return reply(res,404,{error:'project_not_found',requestId},observe,{requestId,req,started});const token=controlPlane.issueProjectToken({accountId:account.id,projectId});return reply(res,200,{token,projectId,entitlements:controlPlane.entitlements(projectId)},observe,{requestId,req,started});}
  ...
  return Object.freeze({server,sessions,billing,revocations,rotateSessionKey:(keyId,key)=>sessions.rotate(keyId,key),retireSessionKey:(keyId)=>sessions.retire(keyId),async listen({host='127.0.0.1',port=0}={}){await new Promise((resolve,reject)=>{server.once('error',reject);server.listen(port,host,resolve);});const address=server.address();return{host,port:address.port,url:`http://${host}:${address.port}`};},async close(){if(server.listening)await new Promise((resolve,reject)=>server.close((error)=>error?reject(error):resolve()));await revocations?.close();}});
}
