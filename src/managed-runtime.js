...
export class ManagedRuntimeManager {
  constructor({ controlPlane, tokenSecret, storageRoot, regions=['local'], maxProjects=100, startupTimeoutMs=15000 }={}) {
    ...
    this.controlPlane=controlPlane;this.secret=key;this.storageRoot=path.resolve(storageRoot??'./syncio-managed-projects');this.regions=[...new Set(regions)];this.maxProjects=maxProjects;this.startupTimeoutMs=startupTimeoutMs;this.instances=new Map();this.tokens=new RotatingTokenAuthority({keys:{managed:key},activeKeyId:'managed',issuer:'syncio-managed'});
  }
  ...
  issueDataToken({projectId,subject,role='owner',ttlSeconds=3600}={}){const project=this.controlPlane.project(projectId);if(!project)throw notFound('project_not_found');return this.tokens.issue({subject:subject??project.accountId,projectId,role,entitlements:this.controlPlane.entitlements(projectId),expiresInSeconds:ttlSeconds});}
  async startProject(projectId,{region=this.regions[0],databaseOptions={}}={}){
    const project=this.controlPlane.project(projectId);if(!project)throw notFound('project_not_found');if(!this.regions.includes(region))throw Object.assign(new Error('unsupported_region'),{code:'unsupported_region'});if(this.instances.has(projectId))return publicState(this.instances.get(projectId));if(this.instances.size>=this.maxProjects)throw Object.assign(new Error('managed_capacity_exceeded'),{code:'managed_capacity_exceeded',statusCode:503});
    const file=path.join(this.storageRoot,region,projectId,'data.syncio.json');const config={projectId,file,secret:this.secret.toString('base64'),region,port:0,databaseOptions};const child=fork(workerFile,[],{stdio:['ignore','pipe','pipe','ipc'],env:{...process.env,SYNCIO_TENANT_CONFIG:Buffer.from(JSON.stringify(config)).toString('base64url')}});
    ...
    try{const ready=await waitFor(child,(message)=>message?.type==='ready'||message?.type==='failed',this.startupTimeoutMs);if(ready.type==='failed')throw Object.assign(new Error(ready.message),{code:ready.code});state.status='running';state.address=ready.address;state.pid=ready.pid;child.once('exit',(code,signal)=>{state.status=code===0?'stopped':'failed';state.exit={code,signal,at:new Date().toISOString()};if(this.instances.get(projectId)===state&&state.status==='stopped')this.instances.delete(projectId);});return publicState(state);}catch(error){this.instances.delete(projectId);child.kill('SIGKILL');throw error;}
  }
  async status(projectId){const state=this.instances.get(projectId);if(!state)throw notFound('runtime_not_found');if(state.status!=='running')return publicState(state);const message=await requestStatus(state.child,5000);state.lastStatus=message;return {...publicState(state),runtime:message};}
  async stopProject(projectId,{forceAfterMs=5000}={}){const state=this.instances.get(projectId);if(!state)return false;if(state.status==='stopped'){this.instances.delete(projectId);return true;}state.status='stopping';state.child.send({type:'shutdown'});const result=await Promise.race([waitForExit(state.child),new Promise((resolve)=>setTimeout(()=>resolve(null),forceAfterMs))]);if(!result)state.child.kill('SIGKILL');this.instances.delete(projectId);return true;}
