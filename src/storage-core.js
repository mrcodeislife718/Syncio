...
export class StorageBackedSyncioDatabase {
  constructor(file, meta, wal, state, intents, options, recovery) {
    ...
    this.syncViews = new Map(Object.entries(meta.syncViews ?? {}));
  ...
  static async open(file, options = {}) {
    ...
    const loaded = await readCheckpoint(target);
    ...
    if (legacy || loaded.source === 'backup' || replay.count) {
      recovery = createRecoveryManifest({
        failure: legacy ? 'legacy_state_migration' : loaded.source === 'backup' ? 'primary_checkpoint_unavailable' : 'wal_replay',
        lastDurableCommit: meta.lastCommitId,
        checkpoint: { source: loaded.source, sequence: loaded.sequence },
        walAccepted: replay.accepted,
        walRejected: [],
        invariants: ['wal_contiguous', 'segment_state_authoritative', 'commit_fabric_verified'],
        finalSequence: meta.sequence,
        stateDigest: await digestState(state, meta.collections)
      });
  ...
  storageStatus() {
    ...
    for (const name of this.meta.collections) {
      const store = this.state.getOpened(name);
      if (store) collections[name] = store.stats();
    }
    return { mode: 'commit-fabric-segmented-mvcc', authoritativeState: 'ssd-segments', fullStateResidentInRam: false, sequence: this.sequence, walEntries: this.wal.listAfter(0).length, checkpointEvery: this.checkpointEvery, commitsSinceCheckpoint: this.commitsSinceCheckpoint, degraded: this.degraded, checkpointError: this.checkpointError, transactions: { ...this.txMetrics }, scheduler: this.scheduler.snapshot(), collections };
  }
  ...
  collection(name) {
    ...
    return Object.freeze({
      async insert(value) { const record = recordValue({ ...value, id: value?.id ?? crypto.randomUUID() }); await db.transaction((tx) => { const collection = tx.collection(name); if (collection.get(record.id)) throw duplicate(record.id); collection.put(record); }); return clone(record); },
      ...
      get(id) { return db.readRecord(name, id); },
      all() { return db.readAll(name); },
  ...
  async transaction(work, { maxRetries = 8, origin = 'local', causalParents = [], partitionId = 'local', sourceChangeId = null } = {}) {
    ...
    if (!Number.isSafeInteger(maxRetries) || maxRetries < 0 || maxRetries > 100) throw new TypeError('invalid transaction retry limit');
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      const tx = new WriteSetTransaction(this);
      let result;
      try { result = await work(tx.api()); } catch (error) { this.txMetrics.aborts++; throw error; }
      if (!tx.hasWrites()) return result;
      try {
        await this.enqueue(() => this.commitTransaction(tx, { origin, causalParents, partitionId, sourceChangeId }));
        this.txMetrics.commits++;
        return result;
      } catch (error) {
        if (error.code !== 'SYNCIO_TRANSACTION_CONFLICT' || attempt === maxRetries) { if (error.code === 'SYNCIO_TRANSACTION_CONFLICT') this.txMetrics.conflicts++; throw error; }
        this.txMetrics.conflicts++;
        this.txMetrics.retries++;
        await new Promise((resolve) => setTimeout(resolve, Math.min(16, 1 << Math.min(attempt, 4))));
      }
    }
  ...
  async commitTransaction(tx, { origin, causalParents, partitionId, sourceChangeId }) {
    ...
    if (!ops.length) { if (sourceChangeId) await this.persistAppliedChangeId(sourceChangeId); return null; }
    const admission = this.scheduler.admit({ priority: PRIORITY.foreground, cost: commitCost(ops) });
    if (admission.decision !== 'admit') throw busy(admission);
    try {
      const baseSequence = this.sequence;
      const resultSequence = baseSequence + ops.length;
      ...
      const fabric = createCommitFabric({ databaseId: this.databaseId, partitionId, sequence: resultSequence, transactionId, origin, mutations, causalParents });
      if (!verifyCommitFabric(fabric)) throw corrupt('commit fabric verification failed');
      const events = ops.map((op, index) => ({
        type: op.after ? (op.before ? 'upsert' : 'insert') : 'remove',
        collection: op.collection,
        id: op.id,
        ...(op.after ? { record: clone(op.after) } : {}),
        ...(op.before ? { before: clone(op.before) } : {}),
        fields: mutations[index].fields,
        changeId: index === 0 && sourceChangeId ? sourceChangeId : crypto.randomUUID(),
        sourceChangeId: index === 0 ? sourceChangeId : null,
        originDatabaseId: this.databaseId,
        commitOrigin: fabric.origin,
        ...
        policyVersion: fabric.policyVersion,
        causalParents: [...fabric.causalParents]
      }));
      await this.wal.append({ databaseId: this.databaseId, baseSequence, resultSequence, events });
      try { await applyEvents(this.state, events); }
      catch (error) { this.degraded = true; const durable = codeError('SYNCIO_DURABLE_COMMIT_RESTART_REQUIRED', 'commit is durable in WAL but state application failed'); durable.cause = error; durable.statusCode = 503; throw durable; }
      this.meta.sequence = resultSequence;
      this.meta.lastCommitId = fabric.commitId;
      for (const event of events) {
        this.meta.recordVersions[keyOf(event.collection, event.id)] = event.sequence;
        this.meta.collectionVersions[event.collection] = event.sequence;
        if (!this.meta.collections.includes(event.collection)) this.meta.collections.push(event.collection);
        this.meta.changes.push(clone(event));
        if (event.sourceChangeId) this.meta.appliedChangeIds = bounded([...this.meta.appliedChangeIds, event.sourceChangeId], this.changeRetention * 2);
      }
      this.meta.collections.sort(); trimChanges(this.meta, this.changeRetention); this.commitsSinceCheckpoint++;
      for (const event of events) this.events.emit('change', clone(event));
      this.events.emit('commit', fabric);
      if (this.commitsSinceCheckpoint >= this.checkpointEvery) this.scheduleCheckpoint();
      return fabric;
    } finally { admission.release?.(); }
  }

  async persistAppliedChangeId(changeId) { if (this.meta.appliedChangeIds.includes(changeId)) return; this.meta.appliedChangeIds = bounded([...this.meta.appliedChangeIds, changeId], this.changeRetention * 2); await this.persistCheckpoint(); }
  ...
  watchChanges({ collection = null, after = this.sequence } = {}, listener) {
    ...
    const status = this.resumeStatus(after); if (!status.resumable) throw streamExpired(status);
    let cursor = after;
    for (const event of this.changesSince(after)) { cursor = event.sequence; if (!collection || event.collection === collection) listener(clone(event)); }
    const handler = (event) => { if (event.sequence <= cursor) return; cursor = event.sequence; if (!collection || event.collection === collection) listener(clone(event)); };
    this.events.on('change', handler); return () => this.events.off('change', handler);
  }
  ...
  subscribeQuery(collection, spec, listener, { id = crypto.randomUUID(), emitInitial = true } = {}) {
    validateCollection(collection); if (typeof listener !== 'function') throw new TypeError('listener required');
    const evaluate = () => this.query(collection, spec); const initial = evaluate();
    const unregister = this.reactive.register(id, { collection, query: spec?.where ?? spec ?? {}, evaluate, initial });
    const onCommit = async (commit) => { for (const result of await this.reactive.applyCommit(commit)) if (result.queryId === id) listener({ type: 'query_result', queryId: id, commitId: commit.commitId, sequence: commit.sequence, value: clone(result.value) }); };
    this.events.on('commit', onCommit);
    if (emitInitial) queueMicrotask(() => listener({ type: 'query_result', queryId: id, commitId: this.meta.lastCommitId, sequence: this.sequence, initial: true, value: clone(initial) }));
    return () => { this.events.off('commit', onCommit); unregister(); };
  }

  async defineSyncView(name, { collection, where = {}, maxDocuments = 100000 } = {}) {
    if (typeof name !== 'string' || !/^[A-Za-z0-9_.-]+$/.test(name) || name.length > 128) throw new TypeError('invalid sync view name');
    validateCollection(collection); if (!Number.isSafeInteger(maxDocuments) || maxDocuments < 1) throw new TypeError('invalid maxDocuments');
    const view = { name, collection, where: clone(where), maxDocuments }; this.syncViews.set(name, view); this.meta.syncViews[name] = clone(view); await this.persistCheckpoint(); return clone(view);
  }
  async dropSyncView(name) { const removed = this.syncViews.delete(name); delete this.meta.syncViews[name]; if (removed) await this.persistCheckpoint(); return removed; }
  ...
  async applyReplicationChange(change, resolver = (_local, remote) => remote) {
    if (!change?.changeId || !change.collection) throw new TypeError('replication change identity required');
    if (this.hasAppliedChange(change.changeId)) return { applied: false, duplicate: true };
    let resolved = null;
    await this.transaction((tx) => { const collection = tx.collection(change.collection); if (change.type === 'remove') { collection.remove(change.id); return; } const incoming = recordValue(change.record); resolved = resolver(collection.get(incoming.id), incoming); if (resolved) collection.put(recordValue(resolved)); }, { origin: change.originDatabaseId ?? 'replication', causalParents: change.commitId ? [change.commitId] : [], sourceChangeId: change.changeId });
    if (!this.hasAppliedChange(change.changeId)) await this.persistAppliedChangeId(change.changeId);
    return { applied: true, duplicate: false, record: resolved ? clone(resolved) : null };
  }
...
export class WriteSetTransaction {
  ...
  collection(name) {
    ...
    return Object.freeze({
      ...
      put(record) { const value = recordValue(record); const key = keyOf(name, value.id); if (!tx.reads.has(key)) tx.reads.set(key, tx.db.meta.recordVersions[key] ?? 0); const prior = tx.writes.has(key) ? tx.writes.get(key).before : tx.db.readRecord(name, value.id); tx.writes.set(key, { collection: name, id: value.id, before: clone(prior), after: clone(value) }); },
      remove(id) { validateId(id); const key = keyOf(name, id); if (!tx.reads.has(key)) tx.reads.set(key, tx.db.meta.recordVersions[key] ?? 0); const current = tx.writes.has(key) ? tx.writes.get(key).after : tx.db.readRecord(name, id); if (!current) return false; const before = tx.writes.has(key) ? tx.writes.get(key).before : current; tx.writes.set(key, { collection: name, id, before: clone(before), after: null }); return true; },
...
async function importLegacy(state, meta, legacy) { meta.databaseId = legacy.databaseId ?? meta.databaseId; meta.sequence = legacy.sequence ?? 0; meta.changes = (legacy.changes ?? []).map(clone); meta.appliedChangeIds = [...(legacy.appliedChangeIds ?? [])]; for (const [name, records] of Object.entries(legacy.collections ?? {})) { const store = await state.collection(name); const operations = Object.values(records).map((record) => ({ type: 'put', record: recordValue(record) })); if (operations.length) await store.applyBatch(operations); meta.collections.push(name); meta.collectionVersions[name] = meta.sequence; for (const record of Object.values(records)) meta.recordVersions[keyOf(name, record.id)] = meta.sequence; } meta.collections = [...new Set(meta.collections)].sort(); }
function normalizeOptions(options) { const changeRetention = options.changeRetention ?? 10000; const checkpointEvery = options.checkpointEvery ?? 256; if (!Number.isSafeInteger(changeRetention) || changeRetention < 1 || !Number.isSafeInteger(checkpointEvery) || checkpointEvery < 1) throw new TypeError('invalid storage options'); return { changeRetention, checkpointEvery, storage: options.storage ?? {}, scheduler: options.scheduler, schedulerOptions: options.schedulerOptions ?? {}, reactiveOptions: options.reactiveOptions ?? {} }; }
function freshMeta(databaseId = crypto.randomUUID()) { return { format: FORMAT, version: 1, databaseId, sequence: 0, lastCommitId: null, collections: [], changes: [], appliedChangeIds: [], recordVersions: {}, collectionVersions: {}, syncViews: {} }; }
function isMeta(value) { return value?.format === FORMAT && value?.version === 1; }
function normalizeMeta(value) { if (typeof value.databaseId !== 'string' || !Number.isSafeInteger(value.sequence)) throw corrupt('invalid storage checkpoint'); return { ...freshMeta(value.databaseId), ...clone(value), collections: [...new Set(value.collections ?? [])].sort(), recordVersions: { ...(value.recordVersions ?? {}) }, collectionVersions: { ...(value.collectionVersions ?? {}) }, syncViews: { ...(value.syncViews ?? {}) } }; }
...
function trimChanges(meta, count) { if (meta.changes.length > count) meta.changes.splice(0, meta.changes.length - count); }
function bounded(values, count) { return [...new Set(values)].slice(-count); }
...
function changedFields(before, after) { if (!before || !after) return ['*']; const keys = new Set([...Object.keys(before), ...Object.keys(after)]); const changed = [...keys].filter((key) => JSON.stringify(before[key]) !== JSON.stringify(after[key])); return changed.length ? changed : ['*']; }
function keyOf(collection, id) { return `${collection}\u0000${id}`; }
function recordValue(value) { if (!value || typeof value !== 'object' || Array.isArray(value) || typeof value.id !== 'string' || !value.id) throw new TypeError('record requires id'); const text = JSON.stringify(value); if (text === undefined) throw new TypeError('record not JSON serializable'); return JSON.parse(text); }
function validateCollection(value) { if (typeof value !== 'string' || !/^[A-Za-z0-9_-]+$/.test(value) || value.length > 128) throw new TypeError('invalid collection'); }
function validateId(value) { if (typeof value !== 'string' || !value || value.length > 512) throw new TypeError('invalid id'); }
function sequence(value) { if (!Number.isSafeInteger(value) || value < 0) throw new TypeError('invalid sequence'); }
...
function applyIntent(tx, mutation) { if (!mutation?.collection) throw new TypeError('invalid intent mutation'); const collection = tx.collection(mutation.collection); if (mutation.type === 'remove') return collection.remove(mutation.id); if (['put', 'upsert', 'insert'].includes(mutation.type)) return collection.put(mutation.record); throw new TypeError('unsupported intent mutation'); }
function matchesWhere(record, where) { return queryRecords([record], { where }).length === 1; }
function duplicate(id) { const error = codeError('SYNCIO_DUPLICATE_ID', `duplicate id ${id}`); error.statusCode = 409; return error; }
function conflict(message) { const error = codeError('SYNCIO_TRANSACTION_CONFLICT', message); error.statusCode = 409; return error; }
function corrupt(message) { return codeError('SYNCIO_CORRUPT_DATABASE', message); }
function busy(details) { const error = codeError('SYNCIO_RESOURCE_BUSY', 'resource admission refused'); error.statusCode = 503; error.details = details; return error; }
function streamExpired(details) { const error = codeError('SYNCIO_STREAM_RESUME_EXPIRED', 'resume cursor expired'); error.details = details; return error; }
