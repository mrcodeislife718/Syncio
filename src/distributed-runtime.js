...
export class GlobalCommitLedger {
  ...
  verify(){return this.verifyDetailed().ok;}
  verifyDetailed(){let previous=null,length=0;if(!fsSync.existsSync(this.journalFile))return{ok:this.state.length===0&&this.state.head===null,length:0,head:null};const text=fsSync.readFileSync(this.journalFile,'utf8');for(const line of text.split('\n')){if(!line)continue;let entry;try{entry=JSON.parse(line);}catch{return{ok:false,length,head:previous};}length++;const{hash,...body}=entry;if(body.index!==length||body.previous!==previous||hash!==digest(body))return{ok:false,length,head:previous};previous=hash;}return{ok:true,length,head:previous};}
