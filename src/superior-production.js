...
import { ResilientProductionSyncioDatabase } from './resilient-production.js';
...
export async function openSuperiorProduction(file,options={}){
  ...
  try{
    ...
    const bounded=await BoundedProductionSyncioDatabase.create(base,metadataFile,metadata,options);
    const db=new ResilientProductionSyncioDatabase(bounded);
