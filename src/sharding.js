...
import { queryDocuments } from './document.js';
...
export class ShardedSyncioDatabase {
  ...
  collection(name){
    ...
    return Object.freeze({
      ...
      query(spec={}){const records=router.shardIds.flatMap((id)=>router.shard(id).collection(name).all());return queryDocuments(records,spec);}
    });
  }
...
export function planShardMigration(records, currentShard, nextShardIds, options={}){
  const ring=new ConsistentHashRing(nextShardIds,options);return records.map((record)=>({id:record.id,fromShard:currentShard,toShard:ring.locate(extractShardKey(record,options.shardKey??'id'))})).filter((item)=>item.fromShard!==item.toShard);
}
